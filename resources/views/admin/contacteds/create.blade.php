...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.contacted.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.contacteds.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user_started') ? 'is-invalid' : '' }}" name="user_started_id" id="user_started_id">
                    @foreach($user_starteds as $id => $entry)
                        <option value="{{ $id }}" {{ old('user_started_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user_ended') ? 'is-invalid' : '' }}" name="user_ended_id" id="user_ended_id">
                    @foreach($user_endeds as $id => $entry)
                        <option value="{{ $id }}" {{ old('user_ended_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Contacted::CONTACT_TYPE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('contact_type') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="contact_type_{{ $key }}" name="contact_type" value="{{ $key }}" {{ old('contact_type', '') === (string) $key ? 'checked' : '' }}>
