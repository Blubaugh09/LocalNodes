...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.smss.update", [$sms->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="message">{{ trans('cruds.sms.fields.message') }}</label>
                <input class="form-control {{ $errors->has('message') ? 'is-invalid' : '' }}" type="text" name="message" id="message" value="{{ old('message', $sms->message) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date_to_send') ? 'is-invalid' : '' }}" type="text" name="date_to_send" id="date_to_send" value="{{ old('date_to_send', $sms->date_to_send) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('time_to_send') ? 'is-invalid' : '' }}" type="text" name="time_to_send" id="time_to_send" value="{{ old('time_to_send', $sms->time_to_send) }}">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('sent_to_men') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="sent_to_men" id="sent_to_men" value="1" {{ $sms->sent_to_men || old('sent_to_men', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('sent_to_woman') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="sent_to_woman" id="sent_to_woman" value="1" {{ $sms->sent_to_woman || old('sent_to_woman', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('send_to_roles') ? 'is-invalid' : '' }}" name="send_to_roles[]" id="send_to_roles" multiple>
                    @foreach($send_to_roles as $id => $send_to_role)
                        <option value="{{ $id }}" {{ (in_array($id, old('send_to_roles', [])) || $sms->send_to_roles->contains($id)) ? 'selected' : '' }}>{{ $send_to_role }}</option>
                    @endforeach
                </select>
