...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.relatedTo.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.related-tos.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('initial_contact') ? 'is-invalid' : '' }}" name="initial_contact_id" id="initial_contact_id">
                    @foreach($initial_contacts as $id => $entry)
                        <option value="{{ $id }}" {{ old('initial_contact_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('related_tos') ? 'is-invalid' : '' }}" name="related_tos[]" id="related_tos" multiple>
                    @foreach($related_tos as $id => $related_to)
                        <option value="{{ $id }}" {{ in_array($id, old('related_tos', [])) ? 'selected' : '' }}>{{ $related_to }}</option>
                    @endforeach
                </select>
