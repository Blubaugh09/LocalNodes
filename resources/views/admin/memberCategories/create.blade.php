...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.memberCategory.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.member-categories.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('members') ? 'is-invalid' : '' }}" name="members[]" id="members" multiple>
                    @foreach($members as $id => $member)
                        <option value="{{ $id }}" {{ in_array($id, old('members', [])) ? 'selected' : '' }}>{{ $member }}</option>
                    @endforeach
                </select>
