    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.update", [$user->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('gender') ? 'is-invalid' : '' }}" type="text" name="gender" id="gender" value="{{ old('gender', $user->gender) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('notes') ? 'is-invalid' : '' }}" name="notes" id="notes">{{ old('notes', $user->notes) }}</textarea>
                @if($errors->has('notes'))
                    <div class="invalid-feedback">
                        {{ $errors->first('notes') }}
                    </div>
            ...
            <div class="form-group">
                <label for="mother_id">{{ trans('cruds.user.fields.mother') }}</label>
                <select class="form-control select2 {{ $errors->has('mother') ? 'is-invalid' : '' }}" name="mother_id" id="mother_id">
                    @foreach($mothers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('mother_id') ? old('mother_id') : $user->mother->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.user.fields.mother_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="father_id">{{ trans('cruds.user.fields.father') }}</label>
                <select class="form-control select2 {{ $errors->has('father') ? 'is-invalid' : '' }}" name="father_id" id="father_id">
                    @foreach($fathers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('father_id') ? old('father_id') : $user->father->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.user.fields.father_helper') }}</span>
            </div>
            <div class="form-group">
